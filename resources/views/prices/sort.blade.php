            ...
            <div class="col-md-12">
                <div class="card" id="app">
                    <div class="card-header clearfix">
                        <h4 class="card-title float-left">Sortuj Cenniki AquaParku {{ $aquaPark->name }}</h4>
                        <a href="{{ route('AquaParkModule::prices', [
                            'aquaPark' => $aquaPark->id
                            ])}}" class="btn btn-primary float-right">
                            <i class="mdi mdi-keyboard-backspace"></i> Do listy
                        </a>
                    </div>
                    <div class="card-body">
                        <list name="aqua-parks-prices" aquaPark="{{ $aquaPark->id }}">
                            {{ csrf_field() }}
                        </list>
                    </div>
                </div>
